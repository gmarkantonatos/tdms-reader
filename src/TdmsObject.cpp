...
TdmsObject::TdmsObject(std::iendianfstream& f, unsigned int version, bool verbose)
: file(f),
d_verbose(verbose),
...
string TdmsObject::timestamp(long long secs, unsigned long long fractionSecs)
{
	...
	char buffer[80];
	sprintf(buffer, "%02d.%02d.%d %02d:%02d:%02d,%f",
...
string TdmsObject::readValue(unsigned int itype, TdmsChannel* channel)
{
	...
	case TdmsChannel::tdsTypeI16: //INT16
		{
			short val;
			file >> val;
			if (channel)
	...
	case TdmsChannel::tdsTypeI32: //INT32
		{
			int val;
			file >> val;
			if (channel)
	...
	case TdmsChannel::tdsTypeI64: //INT64
		{
			long long val;
			file >> val;
			if (channel)
	...
	case TdmsChannel::tdsTypeSingleFloat: //FLOAT32
	...
	case TdmsChannel::tdsTypeDoubleFloat: //FLOAT64
	...
	case TdmsChannel::tdsTypeString: //string
		{
			unsigned int size;
			file >> size;
			string s(size, 0);
	...
	case TdmsChannel::tdsTypeBoolean: //bool
		{
			bool val;
			file >> val;
			if (channel)
	...
	case TdmsChannel::tdsTypeTimeStamp: //time stamp
		{
...
void TdmsObject::readRawDataInfo()
{
	...
	if (flagHasRawData && (rawDataIndex > 0)){
		...
		file >> dimension;//!Array dimension (unsigned 32-bit integer)
		if (dimension != 1)
			dimension = 1;//(In current TDMS file format versions, 1.0 and 2.0, 1 is the only valid value)
...
std::string TdmsObject::getProperty(const std::string& name) const
{
...
long long TdmsObject::getChannelSize() const
{
	...
	if (rawDataType == TdmsChannel::tdsTypeString)
		...
		if (d_format_scalers_vector.empty() || d_raw_data_width_vector.empty())
			return 0;
		return (long long)dataTypeSize(d_format_scalers_vector.front().DAQmxDataType)*d_raw_data_width_vector.front()*dimension*nvalue;
	}
...
bool TdmsObject::readRawData(unsigned long long total_chunk_size, TdmsChannel* channel)
{
	if (nvalue == 0){
	...
	if (channel){
		...
		return channel->readRawData(total_chunk_size, d_verbose);
	}
	return false;
}

bool TdmsObject::readDAQmxData(TdmsChannel* channel, bool verbose)
{
	if (channel)
		return channel->readDAQmxData(d_format_scalers_vector, d_raw_data_width_vector, verbose);
	return false;
}
...
TdmsObject::~TdmsObject()
{
	if (!d_format_scalers_vector.empty())
		vector<FormatChangingScaler>().swap(d_format_scalers_vector);

	if (!d_raw_data_width_vector.empty())
		vector<unsigned int>().swap(d_raw_data_width_vector);
}
