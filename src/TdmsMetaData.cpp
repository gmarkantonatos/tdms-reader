...
TdmsMetaData::TdmsMetaData(TdmsParser* parser, unsigned int versionNumber, bool verbose):
d_verbose(verbose),
d_objectCount(0),
d_channel_cnt(0),
d_chunks(0),
d_version_number(versionNumber),
d_parser(parser)
{
...
{
	TdmsObject *o = new TdmsObject(file, d_version_number, verbose);
...
bool TdmsMetaData::readRawData(unsigned long long total_chunk_size)
{
	if (!total_chunk_size)
		return false;
...
bool TdmsMetaData::readChannelRawData(TdmsChannel *channel, TdmsSegment *seg)
{
